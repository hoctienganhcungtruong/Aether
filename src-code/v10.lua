...
function Aether:AddTab(tabName)
    ...
    function Tab:AddDropdown(text, optionsArray, defaultDisplay, callback)
        callback = callback or function() end
        optionsArray = optionsArray or {}
        local isOpened = false
        
        -- Determine initial display text
        local initialSelection = defaultDisplay or "Select..."
        if not defaultDisplay and #optionsArray > 0 then
            local first = optionsArray[1]
            initialSelection = (type(first) == "table") and (first.Display or first.Text) or tostring(first)
        end
        ...
        ChoiceLabel.Size = UDim2.new(0.5, -35, 1, 0)
        ChoiceLabel.Position = UDim2.new(0.5, 0, 0, 0)
        ChoiceLabel.BackgroundTransparency = 1
        ChoiceLabel.Text = initialSelection
        ChoiceLabel.TextColor3 = Theme.Accent
        ChoiceLabel.TextSize = 13
        ChoiceLabel.Font = Enum.Font.GothamBold
        ChoiceLabel.TextXAlignment = Enum.TextXAlignment.Right
        ChoiceLabel.Parent = Header
        ...
        for idx, option in ipairs(optionsArray) do
            local isTable = type(option) == "table"
            
            -- Extract Display text, Image layout, and the raw payload Value
            local displayString = isTable and (option.Display or option.Text) or tostring(option)
            local optionImage = isTable and option.Image or nil
            local rawPayload = isTable and (option.Value ~= nil and option.Value or option) or option
            
            local OptionBtn = Instance.new("TextButton")
            OptionBtn.Size = UDim2.new(1, 0, 0, 36)
            OptionBtn.BackgroundColor3 = Color3.fromRGB(35, 30, 55)
            OptionBtn.Text = "" 
            ...
            if optionImage then
                ...
                Icon.Image = "rbxassetid://" .. tostring(optionImage)
                Icon.Parent = OptionBtn
                textOffset = 38
            end
            
            local OptionLabel = Instance.new("TextLabel")
            OptionLabel.Size = UDim2.new(1, -textOffset, 1, 0)
            OptionLabel.Position = UDim2.new(0, textOffset, 0, 0)
            OptionLabel.BackgroundTransparency = 1
            OptionLabel.Text = displayString
            OptionLabel.TextColor3 = (displayString == ChoiceLabel.Text) and Theme.Text or Theme.TextMuted
            ...
            OptionBtn.MouseButton1Click:Connect(function()
                ChoiceLabel.Text = displayString
                
                for _, child in ipairs(Content:GetChildren()) do
                    if child:IsA("TextButton") and child:FindFirstChild("Label") then
                        child.Label.TextColor3 = (child.Label.Text == displayString) and Theme.Text or Theme.TextMuted
                    end
                end
                
                isOpened = false
                Arrow.Text = "v"
                tween(Content, TweenInfo.new(0.2), {Size = UDim2.new(1, -20, 0, 0)})
                tween(DropdownFrame, TweenInfo.new(0.2), {Size = UDim2.new(1, 0, 0, 42)})
                task.wait(0.2)
                Content.Visible = false
                
                -- Returns the completely raw data type back to your callback!
                callback(rawPayload)
            end)
        end
        ...
        Header.MouseButton1Click:Connect(function()
            ...
            local targetContentHeight = isOpened and ContentLayout.AbsoluteContentSize.Y or 0
            local targetFrameHeight = isOpened and (ContentLayout.AbsoluteContentSize.Y + 52) or 42
