...
-- SAFE PARENT DETECTION: Falls back gracefully if CoreGui is restricted
...
if RunService:IsStudio() then
    ...
    -- Safe check for exploit environments, otherwise defaults to PlayerGui
    local success, _ = pcall(function() local _ = CoreGui.Name end)
    if success then
        ParentGui = CoreGui:FindFirstChild("RobloxGui") or CoreGui
    else
        ParentGui = PlayerGui
    end
end
...
-- Smooth Dragging Handler (Improved Mobile/PC compatibility)
...
function Aether.new(titleText, initialSize)
    ...
    -- Resize Sensors (Edges drag main frame now)
...
function Aether:AddTab(tabName)
    ...
    -- Safe tab selection: Selects the tab if it's the first one created
